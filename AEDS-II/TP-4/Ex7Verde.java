...
public class Ex7Verde{

//HASH

  //Tamanho reserva
  public static int tamReserva = 0;

  public static Jogador Hash [] = new Jogador[30];
  ...
  public static void inserirHash(Jogador jogador){
	...
	if(tamReserva < Hash.length){
		Hash[tamReserva] = jogador;
		tamReserva++;
	}
    }
  }
  
  public static boolean pesquisarHash(Jogador jogador){
    int altura = jogador.getAltura();
    int pos = altura % 21;
    String nome = jogador.getNome();
    boolean resp = false;
    if(nome.equals(Hash[pos].getNome())){
    	resp = true;
    }
    else{
    	for(int i = 21; i < Hash.length; i++){
		if(nome.equals(Hash[i].getNome())){
        	resp = true;
	    }
	}
    }
  ...
  public static Jogador buscarPorNome(String nome) throws Exception{
  Jogador resp = new Jogador(), temp = new Jogador();
  temp.ler("/tmp/players.csv");
    for(int i = 0; i < temp.lista.length; i++){
      if(nome.equals(temp.lista[i].getNome())){
	      resp = temp.lista[i].clone();
      }
    }
    return resp;
  }
  ...
  public static void main(String [] args) throws Exception{
		...
		entrada = leia.nextLine();
		            ...
		            entrada = leia.nextLine();
		while(!entrada.equals("FIM")){
                  temp = buscarPorNome(entrada); 
//		  System.out.println(temp.getId());  --> teste de funcionalidade
                  if(pesquisarHash(temp)){
