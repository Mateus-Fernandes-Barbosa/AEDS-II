...
void printAlturas(){
  ...
  int i, acumulador = 0, resp;
  float aux;
  
  for(i = 0; i < tamLJogadores; i++){
    acumulador += listaJogadores[i].height;
  }
  
  aux = (float)acumulador / tamLJogadores;
  resp = acumulador / tamLJogadores;
  
  if(aux - resp < 1 && aux - resp >= 0.5){
    resp++;
  }
...
void inserirFim(int x) {
  Player temp;
  if(tamLJogadores >= NMAX){
    temp = removerInicio();
  }
   
   temp = player_searchById(x);
